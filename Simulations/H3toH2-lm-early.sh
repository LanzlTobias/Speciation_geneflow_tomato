...
for i in {1..100}
do
sim_name=$(echo "H3toH2-lm-early_$i")
...
migT=$(echo $Ne | awk '{print (150000/5)/($1*4)}')   ## migration time from H3 to H2
migR=$(echo $Ne | awk '{print 1e-6*$1*4}')          ## migration rate from H3 to H2
...
/data/proj/teaching/NGS_course/Softwares/scrm 24 1 -t $theta -r $rho $size -I 4 2 2 10 10 \
        -em $migT 2 3 $migR -em $divT1 2 3 0\
...
perl ABBA_out_blocker_5Mb.pl D_fd_Fhom_ANGSD_$sim_name.txt > D_fd_Fhom_ANGSD_block_$sim_name.txt

Rscript Jackknife_ABBA_pipe2.R D_fd_Fhom_ANGSD_block_$sim_name.txt D_fd_Fhom_ANGSD_jackknife_$sim_name.txt
